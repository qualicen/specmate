...
public class MatchTreeBuilder {
	...
	public Optional<MatchResultTreeNode> getFirstArgument(MatchResult result) {
		...
		if(name != null) {
			return this.buildTree(result.getSubmatch(name));
		}
		return null;
	}
	...
	public Optional<MatchResultTreeNode> buildTree(MatchResult result) {
		...
		// Binary
		if(isConditionVarible(result) || isVerbObject(result) || isVerbPreposition(result)|| 
				isConjunction(result) || isCondition(result) || isLimitedCondition(result)) {
