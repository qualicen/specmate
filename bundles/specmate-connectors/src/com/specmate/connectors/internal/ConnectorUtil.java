...
public class ConnectorUtil {
	...
	public ConnectorUtil(List<IRequirementsSource> requirementsSources, ITransaction transaction,
	 ...
	 * BATCH_SIZE
	 */
	public void syncRequirementsFromSources() {
		lock.lock();
		try {
					...
					while (greatestUnhandledIndex <= maxIndex) {
						...
						try {
							...
							logService.log(LogService.LOG_ERROR,
									"An error occured while committing synced requirements. Reason:" + e.getMessage(),
									e);
	 ...
	 */
	public void syncRequirementById(String id) {
		lock.lock();
		try {
			...
			for (IRequirementsSource source : requirementsSources) {
				...
				try {
					...
					try {
						...
						logService.log(LogService.LOG_ERROR,
								"An error occured while committing synced requirement. Reason:" + e.getMessage(), e);
						transaction.rollback();
					}

				} catch (Exception e) {
					logService.log(LogService.LOG_ERROR,
							"An error occured synching requirement. Reason: " + e.getMessage(), e);
					transaction.rollback();
				}
			}
		} finally {
