...
public class Scheduler {
	...
	class SchedulerTimerTask extends TimerTask {
		...
		public SchedulerTimerTask(SchedulerTask schedulerTask, ScheduleIterator iterator, Logger logger) {
		...
		public void run() {
			try {
				...
				logger.error("There was an error executing scheduler task", t);
			} finally {
	...
	/**
	 * Terminates this <code>Scheduler</code>, discarding any currently scheduled
	 * tasks. Does not interfere with a currently executing task (if it exists).
	 * Once a scheduler has been terminated, its execution thread terminates
	 * gracefully, and no more tasks may be scheduled on it.
	 * <p>
	 * Note that calling this method from within the run method of a scheduler task
	 * that was invoked by this scheduler absolutely guarantees that the ongoing
	 * task execution is the last task execution that will ever be performed by this
	 * scheduler.
	 * <p>
	 ...
	 * schedule. If times specified by the <code>ScheduleIterator</code> are in the
	 * past they are scheduled for immediate execution.
	 * <p>
	 * 
	 ...
	 *                               scheduler was cancelled, or scheduler thread
	 *                               terminated.
	 */
	public void schedule(SchedulerTask schedulerTask, ScheduleIterator iterator) {
		...
		if (time == null) {
			...
			synchronized (schedulerTask.lock) {
				...
				schedulerTask.timerTask = new SchedulerTimerTask(schedulerTask, iterator, logger);
